...
while ($fila = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $dataRow = array(
        iconv('UTF-8', 'ISO-8859-1', $fila['codigo']),
        iconv('UTF-8', 'ISO-8859-1', strtoupper($fila['descripcion'])),
        iconv('UTF-8', 'ISO-8859-1', $fila['stock']),
        number_format($fila['precio_compra'], 2),
        number_format($fila['precio_venta'], 2),
        iconv('UTF-8', 'ISO-8859-1', $fila['fecha'])
    );

    $aligns = array('C','L','C','R','R','C');
