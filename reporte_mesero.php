...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 12, iconv('UTF-8', 'ISO-8859-1', 'LISTA DE MESEROS'), 0, 1, 'C');
    ...
    function TablaHeader()
    {
        ...
        $tablaAncho = 185;
        $anchoColumna = array(60, 40, 40, 50);
        $posX = (210 - $tablaAncho) / 2;
        $this->SetX($posX);

        // Cabeceras con bordes completos
        $this->Cell($anchoColumna[0], 10, iconv('UTF-8', 'ISO-8859-1', 'NOMBRE'), 1, 0, 'C', true);
        $this->Cell($anchoColumna[1], 10, 'DOCUMENTO', 1, 0, 'C', true);
        $this->Cell($anchoColumna[2], 10, iconv('UTF-8', 'ISO-8859-1', 'TELÉFONO'), 1, 0, 'C', true);
...
while ($fila = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $pdf->Cell($anchoColumna[1], 8, strtoupper($fila['documento']), 1, 0, 'C', true);
    $pdf->Cell($anchoColumna[2], 8, strtoupper($fila['telefono']), 1, 0, 'C', true);
    $pdf->Cell($anchoColumna[3], 8, iconv('UTF-8', 'ISO-8859-1', strtoupper($fila['direccion'])), 1, 1, 'L', true);
